...
def get_answer(driver, question):
    ...
    # Climb up to find the container holding the answer
    try:
        container = question
        for _ in range(3):
            container = container.find_element(By.XPATH, "./..")
            txt = clean_text(container)
            # If the container text is much longer than the question, it's the answer block
            if len(txt) > len(clean_text(question)) + 20:
                return container
    except:
        pass
...
def crawl(driver, url, depth, visited, seen_questions, rows):
    ...
    try:
        ...
        # Find all possible question elements (SAP uses H3)
        questions = driver.find_elements(By.TAG_NAME, "h3")

        for q in questions:
            q_text_raw = clean_text(q)

            # Filter out non-questions or duplicates using seen_questions
            ...
            try:
                if answer_container:
                    ...
                    for a in potential_links:
                        ...
                        if href and "http" in href and "facebook" not in href and href != url and href not in links_urls:
